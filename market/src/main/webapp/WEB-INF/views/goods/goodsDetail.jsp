				...
				<c:forEach var="i" begin="0" end="${fn:length(imageFileList)-1}">
					<li>
						<img width="400" height="400" 
							src="${contextPath}/download.do?goods_id=${goods.goods_id}&fileName=${imageFileList[i].filename}">
					</li>
				</c:forEach>
			</ul>
		</div>
		...
		<div id="member_market">
			...
			<c:choose>
				<c:when test="${not empty marketInfo.market_image}">
					<img style="width:40px; height:40px;" class="profile" src="${contextPath}/downloadMarketImage.do?member_id=${marketInfo.member_id}&market_image=${marketInfo.market_image}">
				</c:when>
				<c:otherwise>
				...
				<c:when test="${goodsNum <= 2}">
					<c:forEach var="item" items="${marketGoodsList}">
						<a href="javascript:void(0)" onclick="detailGoods(${item.goods_id})">
							<img src="${contextPath}/download.do?goods_id=${item.goods_id}&fileName=${item.filename}">
						</a>
					</c:forEach>
				</c:when>
				<c:when test="${goodsNum > 2}">
					<c:forEach var="i" begin="0" end="1">
						<a href="javascript:void(0)" onclick="detailGoods(${marketGoodsList[i].goods_id})">
							<img src="${contextPath}/download.do?goods_id=${marketGoodsList[i].goods_id}&fileName=${marketGoodsList[i].filename}">
						</a>
					</c:forEach>
					<br><br>
