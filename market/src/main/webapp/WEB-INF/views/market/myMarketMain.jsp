		...
		if (chatMemberList.length == 0) {
			...
			for (var i=0; i<chatMemberList.length; i++) {
				var marketInfo = chatMemberList[i];
			 	html += "<img style='width:40px;height:40px;' class='profile' src='${contextPath}/downloadMarketImage.do?member_id="+marketInfo.member_id+"&market_image="+marketInfo.market_image+"'>";
...
function openReviewForm(goods_id) {
	...
	var src = "${contextPath}/download.do?goods_id=" + goods.goods_id + "&fileName=" + goods.filename;
				...
				<c:choose>
						<c:when test="${not empty marketInfo.market_image}">
							<img class="profile" id="preview"
								src="${contextPath}/downloadMarketImage.do?member_id=${marketInfo.member_id}&market_image=${marketInfo.market_image}">
						</c:when>
						<c:otherwise>
	        			...
	        			<div class="goods" id="buyer_goods">
       						<a href="javascript:void(0)" onclick="detailGoods(${buyer_goods.goods_id})">
								<img width="180" height="180" 
									src="${contextPath}/download.do?goods_id=${buyer_goods.goods_id}&fileName=${buyer_goods.filename}" >
							</a>
	        ...
	        <c:forEach var="heart" items="${memberHeartList}">
	        	<div class="goods" id="heart_goods">
		        	<a href="javascript:void(0)" onclick="detailGoods(${heart.goods_id})">
			        	<img width="180" height="180"
			        		src="${contextPath}/download.do?goods_id=${heart.goods_id}&fileName=${heart.filename}">
			        </a>
