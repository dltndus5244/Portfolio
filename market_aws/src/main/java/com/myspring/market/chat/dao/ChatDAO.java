...
public class ChatDAO {
	...
	public int[] selectChatroomListByGoodsId(int goods_id) throws DataAccessException {
		int[] chatroomIdList = new int[10];
		List<ChatroomVO> chatroomList = sqlSession.selectList("mapper.chat.selectChatroomListByGoodsId", goods_id);
		
		for (int i=0; i<chatroomList.size(); i++) {
			int chatroom_id = chatroomList.get(i).getChatroom_id();
			chatroomIdList[i] = chatroom_id;
		}
		return chatroomIdList;
	}
	
	public void deleteChatroom(int goods_id) throws DataAccessException {
		sqlSession.delete("mapper.chat.deleteChatroom", goods_id);
	}
	
	public void deleteMessage(int chatroom_id) throws DataAccessException {
		sqlSession.delete("mapper.chat.deleteMessage", chatroom_id);
	}
}
